...
export default function CaseDetailPage({ caseFile, onBack }: { caseFile: CaseDetail; onBack: () => void }) {
  ...
  return <div className="w-full min-w-0 max-w-full overflow-x-hidden px-3 pb-10 pt-3 sm:px-4 lg:px-6 lg:pt-5">
    <div className="fixed left-3 top-3 z-40 flex w-fit sm:left-6 sm:top-5">
      <button type="button" onClick={onBack} className="group inline-flex h-12 items-center gap-2 rounded-2xl border border-slate-800 bg-slate-950 py-1 pl-1.5 pr-4 text-left text-white shadow-[0_10px_25px_rgba(15,23,42,0.22)] transition duration-300 hover:-translate-y-0.5 hover:bg-indigo-950 hover:shadow-[0_15px_32px_rgba(49,46,129,0.28)] active:translate-y-0 active:scale-[.98] dark:border-indigo-400/25">
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-indigo-200 transition duration-300 group-hover:-translate-x-0.5 group-hover:bg-indigo-400/20 group-hover:text-white"><ChevronLeft size={18}/></span>
        <span><span className="block text-[8px] font-black tracking-[.16em] text-indigo-300">CASE DESK</span><span className="mt-0.5 block text-xs font-bold">案件一覧に戻る</span></span>
      </button>
    </div>
    ...
    <section className="mt-5 overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:bg-[#111a2e]">
      <nav className="flex overflow-x-auto border-b border-slate-200 bg-slate-50/70 px-3 py-2 dark:border-slate-700 dark:bg-[#0c1527]">{([['documents', '資料・書面', FileText], ['precedents', '判例・法令メモ', Landmark], ['meetings', '打合せ記録', MessageSquare]] as const).map(([id, label, Icon]) => <button key={id} type="button" onClick={() => setTab(id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${tab === id ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-200 dark:ring-indigo-500/20' : 'text-slate-400 hover:bg-white/70 hover:text-slate-700 dark:hover:bg-white/[.04] dark:hover:text-slate-200'}`}><Icon size={16}/>{label}</button>)}</nav>
