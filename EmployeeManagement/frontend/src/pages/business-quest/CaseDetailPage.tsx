...
import { CalendarClock, ChevronDown, ChevronLeft, ExternalLink, FileText, Landmark, Mail, MapPin, MessageSquare, Pencil, Phone, Plus, StickyNote, Trash2, UserRound, X } from 'lucide-react'
import api from '../../services/api'
import { caseStatusOptions, formatDateTime, interactionLabels, japanDateValue, statusConfig, statusMap } from './helpers'
import type { CaseCustomSection, CaseDetail, CaseDocument, CaseMeetingLog, CasePrecedent, ClientProfile, DetailTab, InteractionType } from './types'
...
type DialogMode = 'document' | 'precedent' | 'meeting' | 'custom-section' | null
...
export default function CaseDetailPage({ caseFile, onBack }: { caseFile: CaseDetail; onBack: () => void }) {
  ...
  const [selectedCustomSectionId, setSelectedCustomSectionId] = useState<number | null>(null)
  ...
  const [customSectionDraft, setCustomSectionDraft] = useState({ title: '', content: '' })
  ...
  const customSections = currentCase.custom_sections ?? []
  const selectedCustomSection = customSections.find((section) => section.id === selectedCustomSectionId) ?? null
  ...
  const openCustomSectionDialog = () => {
    setCustomSectionDraft({ title: '', content: '' })
    setSaveError(null)
    setDialog('custom-section')
  }
  ...
  const saveRecord = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (dialog === 'custom-section') {
        const response = await api.post<{ custom_section: CaseCustomSection }>(`/case-files/${currentCase.id}/custom-sections`, { title: customSectionDraft.title, content: customSectionDraft.content || null })
        setCurrentCase((value) => ({ ...value, custom_sections: [...(value.custom_sections ?? []), response.data.custom_section] }))
        setSelectedCustomSectionId(response.data.custom_section.id)
      } else if (dialog === 'document') {
        const payload = { title: document.title, category: document.category || 'その他', status: document.status, file_url: document.file_url || null }
  ...
  return <div className="w-full min-w-0 max-w-full overflow-x-hidden px-3 pb-10 pt-32 sm:px-4 md:pt-20 lg:px-6">
    <button type="button" onClick={onBack} className="fixed left-4 top-16 z-30 inline-flex h-12 items-center gap-2.5 rounded-full border border-slate-300/90 bg-white/95 px-4 text-sm font-semibold text-slate-600 shadow-[0_5px_14px_rgba(15,23,42,0.16),inset_0_1px_1px_rgba(255,255,255,0.9)] backdrop-blur-sm transition duration-200 hover:-translate-y-px hover:border-indigo-300 hover:bg-white hover:text-indigo-600 hover:shadow-[0_8px_18px_rgba(15,23,42,0.18)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/60 active:translate-y-0 dark:border-slate-500/70 dark:bg-slate-800/90 dark:text-slate-100 dark:shadow-[0_5px_14px_rgba(0,0,0,0.28),inset_0_1px_1px_rgba(255,255,255,0.08)] dark:hover:border-indigo-400 dark:hover:bg-slate-700 dark:hover:text-white md:left-[19rem] md:top-5" aria-label="案件一覧に戻る"><ChevronLeft size={25} strokeWidth={2}/><span>Back</span></button>
    <header className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.08)] dark:border-slate-700 dark:bg-[#111a2e]">
      <div className="relative overflow-hidden bg-[linear-gradient(118deg,#091024_0%,#172554_52%,#312e81_100%)] px-5 py-6 text-white sm:px-7 sm:py-7">
        <div className="pointer-events-none absolute -right-16 -top-24 h-80 w-80 rounded-full border border-indigo-200/20"/><div className="pointer-events-none absolute right-16 top-5 h-32 w-32 rounded-full bg-violet-400/15 blur-3xl"/>
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex min-w-0 items-center gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl border border-white/15 bg-white/10 text-xl font-black text-white shadow-lg shadow-slate-950/20">{currentCase.client.name.charAt(0)}</div><div className="min-w-0"><p className="text-[10px] font-black tracking-[.18em] text-indigo-200">CASE DOSSIER · {code}</p><h1 className="mt-1.5 truncate text-2xl font-black tracking-tight">{currentCase.client.name}</h1><p className="mt-1 truncate text-sm text-indigo-100/80">{currentCase.title}</p></div></div><div className="flex flex-wrap items-center gap-2"><label className="relative"><span className="sr-only">案件ステータスを更新</span><select aria-label="案件ステータスを更新" value={currentCase.status} disabled={isUpdatingStatus} onChange={(event) => void updateCaseStatus(event.target.value as CaseDetail['status'])} className={`h-10 cursor-pointer appearance-none rounded-xl border bg-white py-0 pl-3 pr-8 text-xs font-bold shadow-sm outline-none transition focus:ring-2 focus:ring-white/45 disabled:cursor-wait disabled:opacity-60 ${status.badge}`}><option value={currentCase.status}>{isUpdatingStatus ? '更新中…' : `● ${status.label}`}</option>{caseStatusOptions.filter((option) => option.value !== currentCase.status).map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select><ChevronDown size={14} className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-500"/></label><button type="button" disabled={isDeletingClient} onClick={() => setIsClientDeleteOpen(true)} className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-rose-200/35 bg-rose-500/10 px-3 text-xs font-bold text-rose-100 transition hover:border-rose-200/60 hover:bg-rose-500/20 disabled:opacity-50"><Trash2 size={14}/>依頼者を削除</button></div></div>
      </div>
      <div className="grid gap-px bg-slate-100 dark:bg-slate-800 sm:grid-cols-2 lg:grid-cols-4"><DossierMeta label="案件種別" value={currentCase.case_type === 'その他' && currentCase.case_type_other ? `その他：${currentCase.case_type_other}` : currentCase.case_type ?? '未分類'}/><DossierMeta label="登録者" value={currentCase.created_by_employee?.full_name ?? '未記録'}/><DossierMeta label="登録日時" value={currentCase.created_at ? formatDateTime(currentCase.created_at) : '—'}/><DossierMeta label="最終更新" value={formatDateTime(currentCase.updated_at)}/></div>
      <ClientContactOverview client={currentCase.client}/>
    </header>

    <section className="mt-5 overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_16px_42px_rgba(15,23,42,0.06)] dark:border-slate-700 dark:bg-[#111a2e]">
      <nav className="flex overflow-x-auto border-b border-slate-200 bg-slate-50/70 px-3 py-2 dark:border-slate-700 dark:bg-[#0c1527]">{([['documents', '資料・書面', FileText], ['precedents', '判例・法令メモ', Landmark], ['meetings', '打合せ記録', MessageSquare]] as const).map(([id, label, Icon]) => <button key={id} type="button" onClick={() => { setTab(id); setSelectedCustomSectionId(null) }} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${tab === id && selectedCustomSectionId === null ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-200 dark:ring-indigo-500/20' : 'text-slate-400 hover:bg-white/70 hover:text-slate-700 dark:hover:bg-white/[.04] dark:hover:text-slate-200'}`}><Icon size={16}/>{label}</button>)}{customSections.map((section) => <button key={section.id} type="button" onClick={() => setSelectedCustomSectionId(section.id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${selectedCustomSectionId === section.id ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-indigo-100 dark:bg-indigo-500/15 dark:text-indigo-200 dark:ring-indigo-500/20' : 'text-slate-400 hover:bg-white/70 hover:text-slate-700 dark:hover:bg-white/[.04] dark:hover:text-slate-200'}`}><StickyNote size={16}/>{section.title}</button>)}<button type="button" onClick={openCustomSectionDialog} className="ml-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-dashed border-slate-300 text-slate-500 transition hover:border-indigo-300 hover:bg-white hover:text-indigo-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 dark:border-slate-600 dark:text-slate-300 dark:hover:border-indigo-400 dark:hover:bg-white/[.04]" aria-label="自由セクションを追加" title="自由セクションを追加"><Plus size={18}/></button></nav>
      <div className="p-4 sm:p-5">
        ...
        {selectedCustomSection ? <CustomSectionTab key={selectedCustomSection.id} section={selectedCustomSection} caseFileId={currentCase.id} onUpdate={(section) => setCurrentCase((value) => ({ ...value, custom_sections: (value.custom_sections ?? []).map((item) => item.id === section.id ? section : item) }))} onDelete={(id) => { setCurrentCase((value) => ({ ...value, custom_sections: (value.custom_sections ?? []).filter((item) => item.id !== id) })); setSelectedCustomSectionId(null) }}/> : <>{tab === 'documents' && <DocumentsTab documents={currentCase.documents} selected={selectedDocument} onSelect={setSelectedDocument} onAdd={() => openDocument()} onEdit={openDocument} onDelete={deleteDocument} deletingId={deletingDocumentId}/>} {tab === 'precedents' && <PrecedentsTab items={currentCase.precedents} expandedId={expandedPrecedentId} onToggle={(id) => setExpandedPrecedentId((current) => current === id ? null : id)} onAdd={() => { setSaveError(null); setDialog('precedent') }}/>} {tab === 'meetings' && <MeetingsTab caseFile={currentCase} items={filteredMeetings} filter={meetingFilter} onFilter={setMeetingFilter} onAdd={() => { setSaveError(null); setDialog('meeting') }}/>}</>}
      </div>
    </section>
    ...
    {dialog === 'custom-section' && <Modal title="自由セクションを追加" onClose={closeDialog}><form onSubmit={saveRecord} className="space-y-4"><Field label="タブ名 *"><input required maxLength={80} value={customSectionDraft.title} onChange={(event) => setCustomSectionDraft({ ...customSectionDraft, title: event.target.value })} className={inputClass} placeholder="例：追加資料・社内確認"/></Field><Field label="初期メモ（任意）"><textarea value={customSectionDraft.content} onChange={(event) => setCustomSectionDraft({ ...customSectionDraft, content: event.target.value })} className={textareaClass} placeholder="このタブで管理したい内容を入力します。"/></Field>{saveError && <p className="text-sm text-rose-600">{saveError}</p>}<Actions onClose={closeDialog} saving={isSaving}/></form></Modal>}
...
function CustomSectionTab({ section, caseFileId, onUpdate, onDelete }: { section: CaseCustomSection; caseFileId: number; onUpdate: (section: CaseCustomSection) => void; onDelete: (id: number) => void }) {
  ...
  const save = async () => {
    try {
      ...
      const response = await api.patch<{ custom_section: CaseCustomSection }>(`/case-files/${caseFileId}/custom-sections/${section.id}`, { title: section.title, content: content || null })
      onUpdate(response.data.custom_section)
    } catch (requestError) { setError(getApiError(requestError, 'セクションを保存できませんでした。')) } finally { setIsSaving(false) }
  }

  const remove = async () => {
    if (!window.confirm(`「${section.title}」を削除しますか？`)) return
    try { await api.delete(`/case-files/${caseFileId}/custom-sections/${section.id}`); onDelete(section.id) } catch (requestError) { setError(getApiError(requestError, 'セクションを削除できませんでした。')) }
  }

  return <section><SectionTop text="業務に合わせて自由に情報を整理できます。" action="保存" onAction={() => void save()}/><div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-[#111a2e]"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-slate-900 dark:text-white">{section.title}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">自由メモ・補足情報</p></div><button type="button" onClick={() => void remove()} className="text-xs font-medium text-rose-600 transition hover:text-rose-700 dark:text-rose-300">削除</button></div><textarea value={content} onChange={(event) => setContent(event.target.value)} className="min-h-64 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-sm leading-6 text-slate-800 outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:border-slate-600 dark:bg-[#0c1527] dark:text-slate-100 dark:focus:ring-indigo-500/20" placeholder="メモ、確認事項、リンクなどを自由に記録します。"/>{error && <p className="mt-3 text-sm text-rose-600">{error}</p>}<div className="mt-3 flex justify-end"><button type="button" disabled={isSaving} onClick={() => void save()} className="h-9 rounded-lg bg-indigo-600 px-3 text-xs font-semibold text-white transition hover:bg-indigo-500 disabled:opacity-60">{isSaving ? '保存中…' : '保存する'}</button></div></div></section>
}
...
function SectionTop({ text, action, onAction }: { text: string; action: string; onAction: () => void }) { return <div className="mb-5 flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800"><div><p className="text-[10px] font-black tracking-[.14em] text-indigo-500">CASE WORKSPACE</p><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{text}</p></div><button type="button" onClick={onAction} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-4 text-xs font-bold text-white shadow-[0_8px_18px_rgba(79,70,229,.22)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_24px_rgba(79,70,229,.3)] active:translate-y-0 active:scale-[.98]"><Plus size={15}/>{action}</button></div> }
...
function ClientContactOverview({ client }: { client: ClientProfile }) {
  return <section aria-labelledby="client-contact-heading" className="border-t border-slate-200 bg-slate-50/60 dark:border-slate-700 dark:bg-[#0c1527]/45"><div className="grid divide-y divide-slate-200 dark:divide-slate-700 md:grid-cols-[13rem_minmax(0,1fr)] md:divide-x md:divide-y-0"><div className="flex items-center justify-between gap-3 px-5 py-3.5 sm:px-6"><div><p id="client-contact-heading" className="text-sm font-semibold text-slate-800 dark:text-slate-100">依頼者連絡先</p><p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-400">Client contact</p></div>{client.nationality && <span className="text-xs font-medium text-slate-500 dark:text-slate-300">国籍：{client.nationality}</span>}</div><div className="grid min-w-0 divide-y divide-slate-200 dark:divide-slate-700 sm:grid-cols-3 sm:divide-x sm:divide-y-0"><ContactItem icon={Phone} label="電話番号" value={client.phone} href={client.phone ? `tel:${client.phone.replace(/[^+\d]/g, '')}` : undefined}/><ContactItem icon={Mail} label="メールアドレス" value={client.email} href={client.email ? `mailto:${client.email}` : undefined}/><ContactItem icon={MapPin} label="住所" value={client.address}/></div></div></section>
}
function ContactItem({ icon: Icon, label, value, href }: { icon: typeof Phone; label: string; value: string | null; href?: string }) { const content = <><Icon size={15} className="mt-0.5 shrink-0 text-slate-400 dark:text-slate-500"/><span className="min-w-0"><span className="block text-[10px] font-medium text-slate-500 dark:text-slate-400">{label}</span><span className="block truncate text-xs font-semibold text-slate-800 dark:text-slate-100">{value || '未登録'}</span></span></>; return href && value ? <a href={href} className="flex min-w-0 gap-2.5 px-4 py-3 transition hover:bg-white/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500/40 dark:hover:bg-white/[.04]">{content}</a> : <div className="flex min-w-0 gap-2.5 px-4 py-3">{content}</div> }
function DossierMeta({ label, value }: { label: string; value: string }) { return <div className="min-w-0 bg-white px-5 py-3.5 dark:bg-[#111a2e]"><p className="text-[9px] font-black tracking-[.14em] text-slate-400 dark:text-slate-500">{label}</p><p className="mt-1 truncate text-xs font-bold text-slate-700 dark:text-slate-200">{value}</p></div> }
function Field({ label, children }: { label: string; children: React.ReactNode }) { if (label === 'バージョン') return null; if (label === '資料名 *' && isValidElement(children)) return <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700 dark:text-slate-200">資料名 *</span><span className="block text-[11px] font-normal text-slate-400 dark:text-slate-500">一覧から選ぶか、該当しない場合はそのまま自由に入力できます。</span>{cloneElement(children, { list: 'document-name-suggestions' } as never)}<datalist id="document-name-suggestions"><option value="申請書"/><option value="パスポート写し"/><option value="在留カード写し"/><option value="住民票"/><option value="証明書"/><option value="委任状"/></datalist></label>; if (label === '種類 *') return null; return <label className="block space-y-1.5"><span className="text-xs font-bold text-slate-700 dark:text-slate-200">{label}</span>{children}</label> }
function Actions({ onClose, saving }: { onClose: () => void; saving: boolean }) { return <div className="flex justify-end gap-2 border-t border-slate-100 pt-4 dark:border-slate-700"><button type="button" onClick={onClose} disabled={saving} className="h-10 rounded-xl px-4 text-sm font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">キャンセル</button><button type="submit" disabled={saving} className="inline-flex h-10 items-center gap-1.5 rounded-xl bg-indigo-600 px-5 text-sm font-bold text-white hover:bg-indigo-500 disabled:opacity-60">{saving && <ButtonSpinner size={15}/>} {saving ? '保存中…' : '保存する'}</button></div> }
