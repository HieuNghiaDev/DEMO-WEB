...
Phase 1A giữ nguyên hệ AI/demo `matters/tasks`. Bản chuẩn bị V2 hiện tại đã bỏ model và tool runtime của hai bảng này; migration cleanup được bảo vệ và chưa mặc định cho phép chạy trên database đang dùng. `case_files` là 案件 chuẩn, `case_tasks` là việc của hồ sơ, `employee_tasks` là công việc nhân viên. Không ánh xạ ID legacy sang hai miền task còn lại. AI page/mascot/provider và approval list/approve/reject được giữ, nhưng hai skill legacy và execution cũ bị vô hiệu hóa. Xem [V2_LOCAL_CLEANUP.md](V2_LOCAL_CLEANUP.md) về gate kiểm thử và trạng thái thực thi. Không upload file, OCR hay gọi Google Drive trong cleanup.
