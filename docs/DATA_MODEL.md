...
erDiagram
    ...
    CLIENTS ||--o{ CLIENT_EMPLOYMENTS : has_history
...
- `case_generated_documents.success_fee_percentage`: tỷ lệ thành công của C‑001, decimal nullable để không ảnh hưởng document generic; giá trị được lưu cùng instance và audit khi thay đổi.
...
C‑001 template v1 và v2 được seed idempotent bằng stable code `C-001`; seeder không tạo thêm document master và không sửa v1. Template v2 là reference template (`参考テンプレート / 事務所承認前`), không phải version tài liệu khách hàng. Workflow C‑001 chính thức chỉ map ba giá trị: `clients.name`, `clients.address` và `case_generated_documents.success_fee_percentage`; tên/địa chỉ luôn được đọc lại từ Case DB, còn operator chỉ chọn mức phí 20/22/24% hoặc giá trị custom hợp lệ. Không đưa các field generic về phạm vi hay điều khoản khác vào C‑001.
...
`generated_document_artifacts` lưu kết quả đã được Google Drive xác nhận: FK case_generated_document_id (restrict delete), artifact_type, storage_provider, external_file_id/url, filename, mime_type, checksum SHA-256 nullable, uploaded_by (user nullable), uploaded_at và timestamps. Luồng PDF generic dùng artifact `pdf`; mỗi version C‑001 chính thức dùng một cặp `source_workbook` (XLSX nguồn đã chốt) và `pdf` (bản xem/in chính), đều nằm trong `作成書類` và không trỏ tới master. Một instance C‑001 chưa có đủ cặp này là working draft, chưa phải version chính thức; danh sách lịch sử chỉ công bố instance có đủ hai artifact. Unique (instance, artifact_type, storage_provider) giữ xác nhận lặp lại idempotent, còn appProperties key có version và loại artifact bảo đảm mỗi v1/v2/v3 dùng file riêng và không ghi đè lịch sử. Không duplicate case_file_id hoặc binary trong MySQL. `CaseGeneratedDocument::artifacts` là hasMany, artifact liên kết instance và uploader. `received_documents` tiếp tục biểu diễn tài liệu nhận vào, không bị dùng sai cho output. Không cần migration mới vì schema hiện tại đã hỗ trợ nhiều loại artifact trên cùng instance.
