...
export default function CollectionEditor({ detail, draft, onChange, errors, employees, employeeError, disabled, canReviewDocuments, editing, onStartEdit, receivedDocuments, documentCreationSection }: {
  ...
  editing: InspectorEditSection | null; onStartEdit: (section: InspectorEditSection) => void; receivedDocuments: ReactNode; documentCreationSection?: ReactNode
}) {
  ...
  return <fieldset ref={editor} className="dc-editor dc-inspector-editor" disabled={disabled}>
    <section className="dc-detail-section dc-inspector-section">
      ...
      </> : <div className="dc-necessity-readonly"><strong className={`dc-status-value is-${detail.necessity.status}`}>{t(`documentCollection.status.necessity.${detail.necessity.status}`)}</strong>{detail.necessity.reason && <p>{detail.necessity.reason}</p>}{detail.necessity.decided_by && <small>{detail.necessity.decided_by.display_name} · {formatDate(detail.necessity.decided_at, true)}</small>}</div>}
    </section>
    ...
    <section className="dc-detail-section dc-inspector-section">
      ...
      ) : <div className="dc-preparation-summary"><div><span>{t('documentCollection.editor.assignee')}</span><strong>{detail.assigned_employee?.display_name ?? t('documentCollection.list.unassigned')}</strong></div><div><span>{t('documentCollection.editor.responseDeadline')}</span><strong>{formatDate(detail.collection.response_deadline, true)}</strong></div><div><span>{t('documentCollection.editor.workPriority')}</span><strong>{t(`documentCollection.status.priority.${detail.collection.priority}`)}</strong></div></div>}
    </section>

    {documentCreationSection}

    {editing === 'receipt' ? <section className="dc-detail-section dc-inspector-section"><SectionHeading label={documentCreationSection ? 'E' : 'D'} title={t('documentCollection.editor.receipt')} action={action('receipt', t('documentCollection.editor.changeReceipt'))}/><div className="dc-form-grid">{select('collection_status', t('documentCollection.filters.collection'), Object.fromEntries(Object.keys(collectionLabels).map(value => [value, t(`documentCollection.status.collection.${value}`)])))}</div>{receivedDocuments}</section> : <details className="dc-compact-section"><summary className="dc-compact-summary"><div className="dc-compact-head"><span>{documentCreationSection ? 'E' : 'D'}</span>{t('documentCollection.editor.receipt')}</div><div className="dc-compact-value">{t('documentCollection.receivedDocuments.title')} {t('cases.count', { count: detail.received_document_count })}</div><ChevronRight size={18} className="dc-compact-chevron" /></summary><div className="dc-compact-body"><dl className="dc-readable-facts"><dt>{t('documentCollection.filters.collection')}</dt><dd><strong>{t(`documentCollection.status.collection.${detail.collection.status}`)}</strong></dd></dl>{receivedDocuments}<div style={{marginTop: 12}}>{action('receipt', t('documentCollection.editor.changeReceipt'))}</div></div></details>}

    {editing === 'review' ? <section className={`dc-detail-section dc-inspector-section ${detail.received_document_count === 0 ? 'is-secondary' : ''}`}><SectionHeading label={documentCreationSection ? 'F' : 'E'} title={t('documentCollection.editor.fulfillmentReview')} action={action('review', canReviewDocuments ? t('documentCollection.editor.changeFulfillmentReview') : t('documentCollection.editor.changeFulfillment'))}/><div className="dc-form-grid">{select('fulfillment_status', t('documentCollection.editor.fulfillment'), Object.fromEntries(Object.keys(fulfillmentLabels).map(value => [value, t(`documentCollection.status.fulfillment.${value}`)])))}{canReviewDocuments ? select('review_status', t('documentCollection.editor.review'), Object.fromEntries(Object.keys(reviewLabels).map(value => [value, t(`documentCollection.status.review.${value}`)]))) : <div className="dc-readonly-field"><span>{t('documentCollection.editor.review')}</span><strong>{t(`documentCollection.status.review.${detail.review_status}`)}</strong></div>}</div></section> : <details className="dc-compact-section"><summary className="dc-compact-summary"><div className="dc-compact-head"><span>{documentCreationSection ? 'F' : 'E'}</span>{t('documentCollection.editor.fulfillmentReview')}</div><div className="dc-compact-value">{t(`documentCollection.status.fulfillment.${detail.fulfillment_status}`)} / {t(`documentCollection.status.review.${detail.review_status}`)}</div><ChevronRight size={18} className="dc-compact-chevron" /></summary><div className="dc-compact-body"><dl className="dc-readable-facts"><dt>{t('documentCollection.editor.fulfillment')}</dt><dd><strong>{t(`documentCollection.status.fulfillment.${detail.fulfillment_status}`)}</strong></dd><dt>{t('documentCollection.editor.review')}</dt><dd><strong>{t(`documentCollection.status.review.${detail.review_status}`)}</strong></dd></dl><div style={{marginTop: 12}}>{action('review', canReviewDocuments ? t('documentCollection.editor.changeFulfillmentReview') : t('documentCollection.editor.changeFulfillment'))}</div></div></details>}

    {editing === 'exception' ? <section className="dc-detail-section dc-inspector-section"><SectionHeading label={documentCreationSection ? 'G' : 'F'} title={t('documentCollection.editor.resultException')} action={action('exception', t('documentCollection.editor.changeException'))}/><div className="dc-form-grid">{select('collection_result', t('documentCollection.editor.resultException'), Object.fromEntries(Object.keys(resultLabels).map(value => [value, t(`documentCollection.status.result.${value}`)])))}</div></section> : <details className="dc-compact-section"><summary className="dc-compact-summary"><div className="dc-compact-head"><span>{documentCreationSection ? 'G' : 'F'}</span>{t('documentCollection.editor.resultException')}</div><div className="dc-compact-value">{detail.collection.result ? <strong className="dc-danger">{t(`documentCollection.status.result.${detail.collection.result}`)}</strong> : t('documentCollection.toolbar.noException')}</div><ChevronRight size={18} className="dc-compact-chevron" /></summary><div className="dc-compact-body"><dl className="dc-readable-facts"><dt>{t('documentCollection.editor.resultException')}</dt><dd><strong className={detail.collection.result ? 'dc-danger' : undefined}>{detail.collection.result ? t(`documentCollection.status.result.${detail.collection.result}`) : t('documentCollection.toolbar.noException')}</strong></dd></dl><div style={{marginTop: 12}}>{action('exception', t('documentCollection.editor.changeException'))}</div></div></details>}
  </fieldset>
}
